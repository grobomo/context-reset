...
def main():
    parser = argparse.ArgumentParser(description="Open a new Claude Code session (never closes calling tab)")
    ...
    parser.add_argument("--target-project", default=None,
                        help="Launch in a different project dir (cross-project session)")
    ...
    # Cross-project: save state in current project, launch in target
    ...
    if not process_detected:
        ...
        log("=== New session launch UNCERTAIN (no new process detected) ===")
        return
    ...
    if new_jsonl:
        ...
        log("=== New session launch UNCERTAIN (no activity detected) ===")
        return

    log("=== New session complete (calling tab preserved) ===")
