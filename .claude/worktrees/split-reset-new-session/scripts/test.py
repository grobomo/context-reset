...
# --- dry-run (context_reset.py) ---
print("\n=== dry-run mode (context_reset.py) ===")
with tempfile.TemporaryDirectory() as d:
    result = subprocess.run(
        [sys.executable, "context_reset.py", "--project-dir", d, "--dry-run"],
        capture_output=True, text=True, cwd=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    )
    test("context_reset dry-run exits 0", result.returncode == 0)
    test("context_reset dry-run prints command", "DRY RUN" in result.stdout)
    test("context_reset dry-run shows shell PID", "shell PID" in result.stdout)

# --- dry-run (new_session.py) ---
print("\n=== dry-run mode (new_session.py) ===")
with tempfile.TemporaryDirectory() as d:
    result = subprocess.run(
        [sys.executable, "new_session.py", "--project-dir", d, "--dry-run"],
        capture_output=True, text=True, cwd=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    )
    test("new_session dry-run exits 0", result.returncode == 0)
    test("new_session dry-run prints command", "DRY RUN" in result.stdout)
    # new_session should NOT mention shell PID (it never kills)
    test("new_session dry-run has no shell PID", "shell PID" not in result.stdout)

# --- split behavior: CLI flags ---
print("\n=== split behavior ===")
import context_reset as cr_module
import new_session as ns_module

# context_reset.py has --stop and --close-tab
cr_parser = cr_module.main.__code__
# Test via subprocess: context_reset accepts --stop
with tempfile.TemporaryDirectory() as d:
    # --stop without a real shell PID just warns and exits
    result = subprocess.run(
        [sys.executable, "context_reset.py", "--project-dir", d, "--stop"],
        capture_output=True, text=True, cwd=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    )
    test("context_reset accepts --stop", result.returncode == 0)
    test("context_reset --stop logs stop mode", "Stop mode" in result.stdout)

# new_session.py should NOT accept --stop, --no-close, --preserve, --close-tab
for flag in ["--stop", "--no-close", "--preserve", "--close-tab"]:
    result = subprocess.run(
        [sys.executable, "new_session.py", "--project-dir", ".", flag],
        capture_output=True, text=True, cwd=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    )
    test(f"new_session rejects {flag}", result.returncode != 0)

# context_reset.py should NOT accept --target-project
result = subprocess.run(
    [sys.executable, "context_reset.py", "--project-dir", ".", "--target-project", "."],
    capture_output=True, text=True, cwd=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
)
test("context_reset rejects --target-project", result.returncode != 0)

# new_session.py should accept --target-project
with tempfile.TemporaryDirectory() as d:
    result = subprocess.run(
        [sys.executable, "new_session.py", "--project-dir", d, "--target-project", d, "--dry-run"],
        capture_output=True, text=True, cwd=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    )
    test("new_session accepts --target-project", result.returncode == 0)
