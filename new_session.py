...
def build_launch_cmd(project_dir, prompt, tab_title, tab_color):
    ...
    if IS_WIN:
        ...
        try:
            ...
            win_dir = project_dir
        # Write bash command to a launcher script. Passing it inline via
        # `bash -c "..."` breaks because wt.exe interprets `;` in the arg list
        # as its own subcommand separator (splits into multiple tabs).
        escaped_file = prompt_file.replace("'", "'\\''")
        escaped_dir = project_dir.replace("'", "'\\''")
        launcher = os.path.join(project_dir, '.claude-launch.sh')
        with open(launcher, 'w', encoding='utf-8') as lf:
            lf.write("#!/usr/bin/env bash\n")
            lf.write("source ~/.profile 2>/dev/null; source ~/.bashrc 2>/dev/null\n")
            lf.write(f"p=$(cat '{escaped_file}')\n")
            lf.write(f"rm -f '{escaped_file}' '{launcher}'\n")
            lf.write(f"cd '{escaped_dir}' && claude --dangerously-skip-permissions \"$p\"\n")
        os.chmod(launcher, 0o755)
        # Use $WT_PROFILE_ID (set by WT in every shell) for correct font/theme.
    ...
    try:
        ...
        while True:
            ...
            if not parent or parent == check:
                break
            check = parent
        # Not trusted — write/overwrite entry matching Claude Code's native format
        ...
        new_entry["hasCompletedProjectOnboarding"] = True
...
def record_session_chain(project_dir, old_jsonl, new_jsonl, reason=None):
...
def main():
    ...
    if new_jsonl:
        ...
        record_session_chain(launch_dir, old_jsonl, new_jsonl, reason=args.reason)
        if close_old:
            shell_pid = find_shell_pid()
            if shell_pid:
